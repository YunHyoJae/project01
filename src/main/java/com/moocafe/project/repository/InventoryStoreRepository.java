...
public interface InventoryStoreRepository extends JpaRepository<InventoryStore, Long> {
    ...
    @Transactional
    @Query("""
                UPDATE InventoryStore s
                SET s.count = s.count + :quantity,
                    s.modifyDate = CURRENT_DATE
                WHERE s.storeId = 1
                  AND s.itemCode = :itemCode
            """)
    int increaseHQStock(@Param("itemCode") String itemCode, @Param("quantity") int quantity);
    ...
    SET s.count = s.count - (
        ...
        FROM returnitem ri
        WHERE ri.returnnumber = :returnNumber
          AND ri.itemcode = s.itemcode
    )
    WHERE s.itemcode IN (
        SELECT ri.itemcode
        FROM returnitem ri
    ...
    AND s.storeid = (
        SELECT so.storeid
        FROM return r
        JOIN storeorder so ON r.ordernumber = so.ordernumber
        WHERE r.returnnumber = :returnNumber
    )
""", nativeQuery = true)
    int decreaseStoreStockByReturnNumber(@Param("returnNumber") String returnNumber);
